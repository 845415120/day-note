...
<!-- ---
prev: '/Javascript/06-对象.md',
next:   '/Javascript/08-函数.md',
--- -->
...
`Date` 对象是一个构造函数，需要使用 `new` 来创建日期对象（实例化）。<br />若没有参数，则默认返回当前系统的当前时间
...
2. charCodeAt()<br />返回该位置的字符的 ASCII 值。一般用于判断用户按下了哪个键。

```javascript
str.charCodeAt(i)
```

3. str[index]<br />H5 新增写法。
...
> [!warning]<br />警告： 尽管 `String.prototype.substr()` 没有严格被废弃 (as in "removed from the Web standards"), 但它被认作是遗留的函数并且可以的话应该避免使用。它并非JavaScript核心语言的一部分，未来将可能会被移除掉。如果可以的话，使用 `substring()` 替代它.
